...
from src.training.HighlightDataset import HighlightDataset
...
def train():
    ...
    dataset = HighlightDataset(CONFIG['CSV'], validation=False, crop_size=CONFIG['crop_size'], 
    ...
    with mlflow.start_run(run_name=CONFIG["model_name"]):
        ...
        for epoch in range(CONFIG["epochs"]):
            ...
            for output in tloader:
                images, sparse = output['gt'].to(CONFIG["device"]), output['six_chan_clipped'].to(CONFIG["device"])
