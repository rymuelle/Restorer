...
class Interpolate(nn.Module):
    def __init__(
        self, size=None, scale_factor=None, mode="bilinear", align_corners=None
    ):
        super().__init__()
        self.size = size
...
class ConvNeXtV2Decoder(nn.Module):
    ...
    def __init__(
        ...
        for idx in range(len(dims) - 1):
            ...
            for jdx in range(depths[idx]):
                ...
                decoder_block_list.append(
                    GeneralizedBlock(
                        _in_dim, dims[idx + 1], drop_path=dp_rates[cur + jdx]
                    )
                )

            decoder_block = nn.Sequential(*decoder_block_list)
        ...
        if use_pixel_shuffle:
            assert dims[-1] / 16 == dims[-1] // 16, (
                f"Input to pixel shuffle must be a multiple of 16, {dims[-1]} dims found."
            )
