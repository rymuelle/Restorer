...
class UNet(nn.Module):
    def __init__(self, img_channel=3, in_channels=6, wf=48, n_l_blocks=[1,1,2], n_h_blocks=[1,1,1], ffn_scale=2):
        ...
        self.ps_down1 = nn.Sequential(
            nn.PixelUnshuffle(2),
            nn.Conv2d((2**2) * in_channels, wf, 1, 1, 0)
        )
        self.ps_down2 = nn.Sequential(
            nn.PixelUnshuffle(4),
            nn.Conv2d((4**2) * in_channels, wf, 1, 1, 0)
        )
        self.ps_down3 = nn.Sequential(
            nn.PixelUnshuffle(8),
            nn.Conv2d((8**2) * in_channels, wf, 1, 1, 0)
        )
        self.conv_01 = nn.Conv2d(in_channels, wf, 3, 1, 1)
...
if __name__== '__main__': 
    ...
    x = torch.randn(1, 6, 1920, 1280).to(device)
    model = UNet(in_channels=6, wf=32, n_l_blocks=[1,2,4], n_h_blocks=[1,1,1], ffn_scale=2).to(device)
#    print(model)
    inp_shape=(6,512, 512)
