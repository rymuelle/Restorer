...
class CombinedPerceptualLoss(nn.Module):
    def __init__(self, lambda_l1=1,
                 lambda_invariant_l1 = 0,
                 lambda_ssim=0.0, lambda_vgg=0.0,
                 lambda_frequency = 0.0, lambda_chroma=0.0,
                 lambda_lpips = 0.0,
    ...
    def forward(self, output, target):
        ...
        if self.lambda_invariant_l1 > 0:
            diff = (target-output).abs
            mean_target = target.mean(dim=(1,2,3))
            mean_target = target.mean(dim=(1,2,3))
            loss += self.lambda_invariant_l1 * self.l1_loss(output, target)

        if self.lambda_ssim > 0:
            loss += self.lambda_ssim * (1 - ms_ssim(output, target, data_range=1.0, size_average=True))
        if self.lambda_vgg > 0:
            vgg_output = apply_gamma(vgg_output)
...
def apply_gamma(_tensor):   
    ...
    tensor[img_mask] = (
        1.055 * torch.pow(tensor[img_mask], 1.0 / 2.4) - 0.055
    )
