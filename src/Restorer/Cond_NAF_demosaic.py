...
from src.Restorer.debayering import Debayer5x5
...
class DemosaicingModelWrapper(nn.Module):
    ...
    def forward(self, x, cond):
        ...
        output = (self.model(x, cond) + debayered).clip(0,1) ** (self.gamma)
        return output
...
class DemosaicingModelLW(nn.Module):
    def __init__(self, **kwargs):
        super().__init__()
        self.demosaicing = Debayer5x5()
        self.ps = nn.PixelShuffle(2)
        self.us = nn.PixelUnshuffle(2)
        self.block = NAFBlock0(16, cond_chans=1)
        self.pw_out = nn.Conv2d(4, 3, 1)
        

    def forward(self, rggb, cond):

        bayer = self.ps(rggb, 2)
        debayered = self.demosaicing(bayer)
        shuffled_bayer = self.us(debayered)
        x = torch.cat([rggb, shuffled_bayer], dim=1)
        x = self.block((x, cond))
        x = self.ps(x)
        x = self.pw_out(x)
        output = x + debayered
...
class DemosaicingModelWrapperSequential(nn.Module):
    def __init__(self, **kwargs):
        self.gamma = 1
        if 'gamma' in kwargs:
            self.gamma = kwargs.pop('gamma')
    ...
    def forward(self, x, cond):
        ...
        bayer = nn.functional.pixel_shuffle(x, 2)
        debayered = self.demosaicing(bayer)
        output = (self.model(debayered, cond) + debayered).clip(0,1) ** (self.gamma)
        return output
    

def make_full_model_RGGB_Demosaicing_Sequential(params, model_name=None):
    
    model = DemosaicingModelWrapperSequential(**params)
    if not model_name is None:
        state_dict = torch.load(model_name, map_location="cpu")
        model.load_state_dict(state_dict)
    return model
