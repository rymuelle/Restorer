...
class SyntheticNoiseDataset(Dataset):
    def __init__(self, csv, crop_size=256, buffer=10, validation=False, resize=1, clipping_threshold=0.8):
        super().__init__()
    ...
    def __getitem__(self, i):
        ...
        sigmas = np.array([[[row['rsigma']]], [[row['gsigma']]], [[row['bsigma']]]])
        alpha  =  row['iso'] / 204800.0

        sigmas = (gt_image * alpha ) * sigmas * 12 + 0.003 #* sigmas / sigmas.mean()
        noise = np.random.randn(*gt_image.shape) * sigmas
        mask = get_mask(deg_pattern, self.masks)
        corrupted = (gt_image + noise).clip(0, 1)

        W, H = self.crop_size + 6, self.crop_size + 6
        ...
        six_chan_corrupted = np.concat([corrupted*mask, mask], axis=0)

        output = {
            "gt": torch.from_numpy(gt_image).to(torch.float32).clamp_(0.0, 1.0),
            "corrupted": torch.from_numpy(corrupted).to(torch.float32).clamp_(0.0, 1.0),
            "six_chan": torch.from_numpy(six_chan).to(torch.float32).clamp_(0.0, 1.0),
            "six_chan_corrupted": torch.from_numpy(six_chan_corrupted).to(torch.float32).clamp_(0.0, 1.0),
            "ccm": torch.from_numpy(deg_ccm).to(torch.float32),
            "iso": torch.tensor(row['iso']).to(torch.float32),
        }
        return output