...
from src.training.SyntheticNoiseDataset import SyntheticNoiseDataset
...
CONFIG = {
    "model_name": "DemoNAFNetMamba_synthetic",
    "experiment_name": "BaseDenoisingSynthetic",
    "batch_size": 16,
    "lr": 5e-4,
    ...
    "width": 32,
    "middle_blk_num": (2, 0),
    "enc_blk_nums":[(0, 0), (0, 0), (2, 0), (2, 0)],
    "dec_blk_nums":[(1, 0), (1, 0), (0, 0), (0, 0)],
...
def train():
    ...
    dataset = SyntheticNoiseDataset(CONFIG['CSV'], validation=False, crop_size=CONFIG['crop_size'])
    ...
    with mlflow.start_run(run_name=CONFIG["model_name"]):
        ...
        for epoch in range(CONFIG["epochs"]):
            ...
            for output in tloader:
                ...
                images, sparse = output['gt'].to(CONFIG["device"]), output['six_chan_corrupted'].to(CONFIG["device"])
            ...
            with torch.no_grad():
                ...
                for output in vloader:
                    ...
                    # mono_noise = output['mono_noise_proportion'].to(CONFIG["device"])
                    # ccm = output['ccm'].to(CONFIG["device"])
                    # images = images * (1 + mono_noise * CONFIG['added_noise'])
                    output = model(sparse)
                    loss = criterion(output, images)
                    if CONFIG['SWL_scale'] > 0:
                        tloss = texture_criteria(output, images) * CONFIG['SWL_scale']
